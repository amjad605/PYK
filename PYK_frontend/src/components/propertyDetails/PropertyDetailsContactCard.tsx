...
const PropertyDetailsContactCard: React.FC<PropertyDetailsContactCardProps> = ({
    property,
}) => {
    return (
                    ...
                    <div className="flex-1 p-4 bg-white/10 backdrop-blur-sm rounded-xl space-y-3 border border-white/20">
                        ...
                        {property.price.paymentPlan?.installments && (
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-semibold">
                                    {property.price.paymentPlan.installments.year} years
                                </span>
                            </div>
                        )}
                        ...
                        {property.listingType === ("Rent" as ListingType) && (
                            <div className="w-full flex justify-center py-6">
                                <div className="rounded-2xl border border-gray-200 shadow-lg p-4 sm:p-6">
                                    <Calendar
                                        numberOfMonths={2}
                                        className="rounded-lg border-none shadow-none px-10 py-14"
                                    />
                                </div>
                            </div>
                        )}
